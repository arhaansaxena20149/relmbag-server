...
set PYTHON_EXE=
for /f "tokens=*" %%i in ('where /q py ^&^& py -3.12 -c "import sys; print(sys.executable)" 2^>nul') do set PYTHON_EXE=%%i
if not defined PYTHON_EXE (
    for /f "tokens=*" %%i in ('where /q py ^&^& py -3.11 -c "import sys; print(sys.executable)" 2^>nul') do set PYTHON_EXE=%%i
)
...
"%PYTHON_EXE%" -m pip install PyQt5 Pillow requests bcrypt flask PyInstaller --trusted-host pypi.org --trusted-host files.pythonhosted.org
